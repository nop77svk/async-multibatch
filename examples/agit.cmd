...
set /a sk.nop77svk.async_multibatch.cfg.thread_pool_size=l_cpu_times*%NUMBER_OF_PROCESSORS%

set sk.nop77svk.async_multibatch.cfg.start_minimized=yes
...
if "x%NOP77SVK.AGIT.PATHS%"=="x" (
	echo.ERROR: NOP77SVK.AGIT.PATHS environment variable not set or empty
	echo.
	echo.Set up the env. variable to a semicolon-delimited list of paths to be traversed in
	echo.a search for GIT repositories. If you're lazy, simply fill the variable with C:\ if
	echo.you want to have this script traverse your whole C: disk.
	exit /b 1
)

set MODUS_OPERANDI=async
if "x%1"=="xdiff" set MODUS_OPERANDI=sync

if not "x%1"=="x" (
	echo.Traversing GIT repositories...
	for %%r in (%NOP77SVK.AGIT.PATHS%) do (
		pushd %%r
		for /r  /d %%d in (.git) do (
			if exist %%d (
				echo. * %%d
	...
	echo.Done traversing GIT repositories

	call :serialization_point

	if %MODUS_OPERANDI%==async (
		for %%o in (%TEMP%\sk.nop77svk.async_multibatch.%l_run_id%.*.out) do (
			if exist "%%~o" (
				echo.------------------------------------------------------------------------------
				type "%%~o"
			)
		...
		del %TEMP%\sk.nop77svk.async_multibatch.%l_run_id%.*.out
	)
) else (
	echo.ERROR: No GIT command supplied
	exit /b 1
)
...
if %MODUS_OPERANDI%==async (
	echo.--- %CD% > %TEMP%\sk.nop77svk.async_multibatch.%l_run_id%.%l_child_id%.out
	git.exe %* >> %TEMP%\sk.nop77svk.async_multibatch.%l_run_id%.%l_child_id%.out
) else (
	git.exe %*
)
